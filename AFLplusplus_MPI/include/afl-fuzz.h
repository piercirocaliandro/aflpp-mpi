...
#define FOREIGN_SYNCS_MAX 32U
	  ...
	  u64 mpi_send_time;
	  u64 mpi_recv_time;
	  u64 mpi_nsend;  // counts the number of single sends done by the fuzzer
	  // Used to determine if we need to enter in AMMISC mode
	  u64 last_time;
	  double mpi_cvg;
	  u64 n_ammuina;
	  u64 saved_ammuina;
	  u64 avg_ammuina;
	  u64 curr_ammuina_num;

	  u64 hash_idx;
	  u64 hash_change_cost;
	  int required_change_hash;
	  unsigned long *utility_directory;
	  unsigned char *send_list;
	  unsigned char *recv_list;
    ...
    int32_t *h_cvgs;
    int32_t h_cvgs_count;
    struct queue_entry **mpi_priority_queue;
    int32_t last_mpi_elem;
    int32_t mpi_priority_queue_size;
    u8 is_cvg_window_init; 
 
    int *cluster;
    uint32_t mpi_cluster_size;
    int mpi_node_type;
    int cluster_main;
    bool is_mpi_main_node;
    int num_hashes;
#endif
} afl_state_t;
...
extern void afl_write_log(char *msg, char *log_name);

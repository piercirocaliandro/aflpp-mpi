...
compile_binary()
{
        cd $1
        make clean
        CC=$CC CXX=$CXX ./configure --disable-shared --with-harfbuzz=no --with-bzip2=no --with-png=no --prefix="$1/../build"
        make all -j$(nproc)
        make install
        cd ..
        $CXX $1/src/tools/ftfuzzer/ftfuzzer.cc $1/objs/.libs/libfreetype.a -I build/include -o $INSTRUMENTED_BINS$2 $HOME/afl_mpi/libAFLDriver.a
        if [[ $3 ]]; then
...
if [[ $# -lt 2 ]]; then
        echo "Usage: ./compile_bins <path> <bin_name>"
fi
compile_bins $1 $2

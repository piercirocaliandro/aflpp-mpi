...
int main(int argc, char **argv_orig, char **envp) { 
  ...
  afl_state_init(afl, map_size);
	...
	afl->mpi_world_size = size;

#ifdef MPI_DYNAMIC
	// Initalize utility directory and send_list
	afl->utility_directory = calloc(afl->mpi_world_size, sizeof(unsigned long));
	afl->send_list = calloc(afl->mpi_world_size, sizeof(unsigned char));
	for (int i = 0; i < afl->mpi_world_size; i++) {
		afl->send_list[i] = 1;
	}
	afl->recv_list = calloc(afl->mpi_world_size, sizeof(unsigned char));
	for (int i = 0; i < afl->mpi_world_size; i++) {
		afl->recv_list[i] = 1;
	}
#endif
    ...
    afl->total_sync_time = 0;

    int my_rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &my_rank);
	afl->world_rank = my_rank;


	afl->mpi_cvg = 0.0;
	afl->last_time = get_cur_time();
  	afl->n_ammuina = 0;
  	afl->saved_ammuina = 0;
	afl->avg_ammuina = 10;
	afl->curr_ammuina_num = 0;
  	afl->hash_idx = 0;
  	afl->hash_change_cost = 0;
	afl->required_change_hash = 0;

#ifdef HIERARCHICAL
    afl->mpi_node_type = -1;  // By default, the type is other

    // Setup the priority queue
    afl->mpi_priority_queue = NULL;
    afl->mpi_priority_queue_size = 0;
    afl->last_mpi_elem = 0;

    // Highest coverage elements, will be exchanged with any other node
    afl->h_cvgs_count = 10;
    afl->h_cvgs = calloc(afl->h_cvgs_count, sizeof(int32_t));
    for (int i =0; i < afl->h_cvgs_count; i++)
        afl->h_cvgs[i] = -1;
...
#if defined(USE_MPI) && defined(HIERARCHICAL)
        afl->mpi_node_type = NODE_OLD_QUEUE;
#endif
...
#if defined(USE_MPI) && defined(HIERARCHICAL)
        afl->mpi_node_type = NODE_EXPLOIT;
#endif
...
#if defined(USE_MPI) && defined(HIERARCHICAL)
        afl->mpi_node_type = NODE_EXPLORE;
#endif
  ...
  afl->start_time = get_cur_time();
...
#if defined(USE_MPI) && defined(HIERARCHICAL)
    // Discover the cluster for this fuzzing run
    //
    if (afl->mpi_node_type == -1) {
   	mpi_write_log("Please configure the correct node type\n");
       exit(1);	
    }
    ...
    for ( int i = 0; i < afl->mpi_world_size; i++ ) {
        if ( i != afl->world_rank )
            MPI_Isend(&afl->mpi_node_type, 1, MPI_UNSIGNED, i, NODE_TYPE, MPI_COMM_WORLD, &req);
     }

     int count = 0;
     int curr_type = -1;
     for (int rank = 0; rank < afl->mpi_world_size; rank++) {
         if (rank != afl->world_rank) {
             MPI_Recv(&curr_type, 1, MPI_UNSIGNED, rank, NODE_TYPE, MPI_COMM_WORLD,
                     MPI_STATUS_IGNORE);
             if ( curr_type == afl->mpi_node_type ) {
                 if ( afl->cluster == NULL ) {
                     afl->cluster = (int *)calloc(1, sizeof(int));
                 } else {
                     afl->cluster = (int *)realloc(afl->cluster, sizeof(int) * (count + 1));
                 }
                 (afl->cluster)[count] = rank;
                 count++;
             }
         }
     }
     afl->mpi_cluster_size = count + 1;  // +1 since we are counting the node itself
#endif
...
#if defined(USE_MPI) && defined(HIERARCHICAL)
        // Gives priority to elements that have been exchanged via MPI messages
        for (; afl->last_mpi_elem < afl->mpi_priority_queue_size; afl->last_mpi_elem++) {
            afl->current_entry = afl->last_mpi_elem;
            afl->queue_cur = afl->mpi_priority_queue[afl->current_entry];
            fuzz_one(afl);
        }
...
void afl_write_log(char *msg, char *log_name)
{

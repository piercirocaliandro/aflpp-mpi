...
void add_to_queue(afl_state_t *afl, u8 *fname, u32 len, u8 passed_det, u8 from_mpi) {
...
#ifdef USE_MPI
  if (from_mpi) {
	afl->mpi_priority_queue_size++;
	if (afl->mpi_priority_queue == NULL) {
		afl->mpi_priority_queue = (struct queue_entry **)calloc(1, sizeof(struct queue_entry *));
	} else {
		afl->mpi_priority_queue = realloc(afl->mpi_priority_queue, afl->mpi_priority_queue_size*sizeof(struct queue_entry *));
	}
	afl->mpi_priority_queue[afl->mpi_priority_queue_size-1] = q;
  	q->id = afl->mpi_priority_queue_size - 1;

...
void __mpi_diffuse_testcase(int32_t delta, unsigned char *buf, size_t len,
                            int src, afl_state_t *afl);
u8 __is_in_mpi_cluster(int, afl_state_t *);
void __init_cvg_window(int32_t, afl_state_t *);
#endif
...
fuzz_run_target(afl_state_t *afl, afl_forkserver_t *fsrv, u32 timeout) {
...
#endif
#if defined(USE_MPI) && defined(AMMUINA)
  int flag = 1;
  MPI_Status status;
  int ammuina = 0;
  while (flag) {
    MPI_Iprobe(MPI_ANY_SOURCE, FACIMM_AMMUINA, MPI_COMM_WORLD, &flag, &status);
    if (flag) {
      MPI_Recv(NULL, 0, MPI_UNSIGNED, status.MPI_SOURCE, FACIMM_AMMUINA,
  ...
  if (ammuina) {
    ...
    timer_uint mpi_start = timer_hr_new();
    
    mpi_write_log("Starting ammuina\n");
    mpi_ammuina_send(afl->world_rank, afl->mpi_world_size);
    afl->last_time = cur_time;

    timer_uint mpi_end = timer_hr_value(mpi_start);
    afl->total_sync_time += mpi_end;

    sync_fuzzers(afl);  // Trigger sync_fuzzers after AMMUINA
  }
#endif
  return res;
}
 ...
 *
 *  TODO: with MPI messages, exchange test cases with other fuzzers without
 * reading from FS.
 * */
...
void sync_fuzzers(afl_state_t *afl) {
...
#ifdef USE_MPI
  /* MPI-receive every message in the buffer, by first probing the COMM_WORLD
   * and then allocating elements in the stack.
   * */
  timer_uint mpi_start = timer_hr_new();
  int flag = 1;
  int64_t curr_cvg = 0;
  ...
  //int32_t prev_cvg = count_non_255_bytes(afl, afl->virgin_bits);

  while (flag) {
    synced = 1;

    //mpi_write_log_int(synced);

    MPI_Status status;
    MPI_Iprobe(MPI_ANY_SOURCE, MPI_SYNC, MPI_COMM_WORLD, &flag, &status);


    if (flag) {
      int len = 0;
      MPI_Get_count(&status, MPI_UINT8_T, &len);
      void *the_buf = malloc(len);
      MPI_Recv(the_buf, len, MPI_UINT8_T, status.MPI_SOURCE, MPI_SYNC, MPI_COMM_WORLD,
               &status);

      int32_t prev_cvg = count_non_255_bytes(afl, afl->virgin_bits);

      u32 new_len = write_to_testcase(afl, (void **)&the_buf, len, 1);
      u8 fault;
      fault = fuzz_run_target(afl, &afl->fsrv, afl->fsrv.exec_tmout);


      int32_t post_cvg = count_non_255_bytes(afl, afl->virgin_bits);
      curr_cvg += (post_cvg - prev_cvg);
      ...
      int afl_ret = save_if_interesting(afl, the_buf, new_len, fault, from_mpi);
      afl->queued_imported += afl_ret;

#if defined(USE_MPI)
      if (afl_ret) {
        save_to_cache(the_buf, new_len, (post_cvg - prev_cvg));

#if defined(HIERARCHICAL)
        int32_t delta = post_cvg - prev_cvg;
        if (likely(!afl->is_cvg_window_init))
          __init_cvg_window(delta, afl);
        else
          __mpi_diffuse_testcase(delta, the_buf, new_len, status.MPI_SOURCE,
                                 afl);
#endif
      }
#endif
...
#ifdef MPI_DYNAMIC
      if ((post_cvg - prev_cvg) > 0) {
        afl->utility_directory[status.MPI_SOURCE] += (post_cvg - prev_cvg);
      } else {
        afl->utility_directory[status.MPI_SOURCE]--;
      }
#endif
    }
  }
...
#ifdef MPI_DYNAMIC
  if (synced) {
    for (int i = 0; i < afl->mpi_world_size; i++) {
      if (i != afl->world_rank && afl->utility_directory[i] <= 0 &&
          afl->recv_list[i]) {
        MPI_Request req;
        MPI_Isend(NULL, 0, MPI_UNSIGNED, i, REMOVE_FROM_SEND, MPI_COMM_WORLD,
                  &req);
        afl->recv_list[i] = 0;
      }
    }

    // Now, verify if someone asked us to remove itself from the send list
    int flag = 1;
    while (flag) {
      MPI_Status status;
      MPI_Iprobe(MPI_ANY_SOURCE, REMOVE_FROM_SEND, MPI_COMM_WORLD, &flag,
                 &status);
      if (flag) {
        MPI_Recv(NULL, 0, MPI_UNSIGNED, status.MPI_SOURCE, REMOVE_FROM_SEND,
                 MPI_COMM_WORLD, &status);
        afl->send_list[status.MPI_SOURCE] = 0;
      }
    }
  }
#endif
  ...
  if (synced) {
    ...
    if ((curr_cvg - afl->ammuina_cvg) <= DELTA_CVG &&
        (cur_time - afl->last_time) >= DELTA_TIME) {
      MPI_Request ctr_req;

      int size = 0;
      MPI_Comm_size(MPI_COMM_WORLD, &size);
      for (int i = 0; i < size; i++) {
        MPI_Isend(NULL, 0, MPI_UNSIGNED, i, FACIMM_AMMUINA, MPI_COMM_WORLD,
                  &ctr_req);
      }
      afl->last_time = cur_time;
    }
    afl->ammuina_cvg = curr_cvg;
  }

#endif
  // Finally check if we can free and stop recv time count
  mpi_sent_elem_free(&the_sent_msg_head);
  timer_uint mpi_end = timer_hr_value(mpi_start);
  afl->total_sync_time += mpi_end;
  ...
  while ((sd_ent = readdir(sd))) {
    ...
    // Start afl FS syc timer
    timer_uint afl_sync_start = timer_hr_new();
    ...
    for (o = m; o < n; o++) {
      ...
      timer_uint afl_sync_end = timer_hr_value(afl_sync_start);
      afl->total_sync_time += afl_sync_end;
      ...
      if (st.st_size && st.st_size <= MAX_FILE) {
        ...
        afl->sync_messages += 1;
        afl->queued_imported += save_if_interesting(afl, mem, new_len, fault, 0);
        show_stats(afl);
        afl->syncing_party = 0;
...
void __mpi_diffuse_testcase(int32_t delta, unsigned char *buf, size_t len,
                            int src, afl_state_t *afl) {
  int32_t min = delta;
  int min_idx = -1, num_lowest = 0;
  for (int32_t idx = 0; idx < afl->h_cvgs_count; idx++) {
    if (afl->h_cvgs[idx] < min) {
      min = afl->h_cvgs[idx];
      min_idx = idx;
    }
    if (afl->h_cvgs[idx] < delta) {
      num_lowest++;
    }
  }
  if (min_idx != -1) {
    if (num_lowest >= afl->h_cvgs_count >> 1) {
      // Broadcast send the testcase to anyone in other clusters excpect the
      // node

      afl_mpi_sent_msg_t *new_node = calloc(1, sizeof(afl_mpi_sent_msg_t));
      new_node->buf = malloc(sizeof(unsigned char) * len);
      memcpy(new_node->buf, buf, len);
      new_node->next = NULL;

      new_node->requests = malloc(afl->mpi_cluster_size * sizeof(MPI_Request));
      for (u32 i = 0; i < afl->mpi_cluster_size; i++) {
        if (afl->cluster[i] != src) {
          MPI_Isend(new_node->buf, len, MPI_UINT8_T, afl->cluster[i], MPI_SYNC,
                    MPI_COMM_WORLD, &(new_node->requests[i]));
          new_node->n_requests++;
        } else {
          new_node->requests[i] = MPI_REQUEST_NULL;
        }
      }
      mpi_sent_elem_insert(new_node);
    }
    afl->h_cvgs[min_idx] = delta;
  }
}

u8 __is_in_mpi_cluster(int rank, afl_state_t *afl) {
  for (uint32_t i = 0; i < afl->mpi_cluster_size; i++)
    if (rank == afl->cluster[i])
      return 1;
  return 0;
}

void __init_cvg_window(int32_t delta, afl_state_t *afl) {
  int i = 0;
  for (; i < afl->h_cvgs_count; i++) {
    if (afl->h_cvgs[i] == -1) {
      afl->h_cvgs[i] = delta;
      break;
    }
  }
  if (i == afl->h_cvgs_count) {
    afl->is_cvg_window_init = 1;
  }
}

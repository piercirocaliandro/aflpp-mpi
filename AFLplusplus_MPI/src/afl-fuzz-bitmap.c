 ...
 * 
 * TODO: send the testcase in broadcast with MPI message to all the other
 * fuzzers, so that no one has to read the distributed file system to sync.
*/

u8 __attribute__((hot)) save_if_interesting(afl_state_t *afl, void *mem,
                                            u32 len, u8 fault, u8 from_mpi_sync) {
  ...
  save_to_queue:
	...
	// Send only in case the testcase gives new coverage
	if (new_bits == 2) {
		...
		memcpy(new_node->buf, mem, len);
		new_node->next = NULL;
#ifdef HASH
		u64 target = mpi_hash_wrap(new_node->buf, len, afl->mpi_world_size);

		if (target != afl->world_rank) {
			new_node->requests = malloc(sizeof(MPI_Request));
			MPI_Isend(new_node->buf, len, MPI_UINT8_T, target, MPI_SYNC, MPI_COMM_WORLD, new_node->requests);
			new_node->n_requests = 1;
        ...
        int start = 0;
        int idx = 0;
        int cmax = afl->mpi_cluster_size;
        for (; start <= (afl->mpi_world_size - afl->mpi_cluster_size); start += afl->mpi_cluster_size) {
            uint64_t target = mpi_multi_hash_wrap(new_node->buf, len, start, cmax);

	    if (target != afl->world_rank) { 
            	MPI_Isend(new_node->buf, len, MPI_UINT8_T, target, MPI_SYNC, MPI_COMM_WORLD,
                    	&(new_node->requests[idx]));
            	new_node->n_requests++;
	    } else {
		    new_node->requests[idx] = MPI_REQUEST_NULL;
	    }
 
            cmax += afl->mpi_cluster_size;
            idx++;
        }
		mpi_sent_elem_insert(new_node);
#endif
...
#ifdef MPI_DYNAMIC
		new_node->requests = malloc(afl->mpi_world_size*sizeof(MPI_Request));
		for (int i = 0; i < afl->mpi_world_size; i++) {
			if ((i != afl->world_rank) && (afl->send_list[i])) {
				MPI_Isend(new_node->buf, len, MPI_UINT8_T, i, MPI_SYNC, MPI_COMM_WORLD, &(new_node->requests[i]));
				new_node->n_requests++;
			}
			else {
    ...
    add_to_queue(afl, queue_fn, len, 0, from_mpi_sync);
    if (from_mpi_sync) return 0;
  ...
  switch (fault) {
    ...
    keep_as_crash:
      ...
      ++afl->saved_crashes;
/*#ifdef USE_MPI
	  mpi_write_log("Saved one crash on filesystem\n");
#else
	afl_write_log("Saved one crash on filesystem\n", afl->log_name);
#endif*/
#ifdef INTROSPECTION
      if (afl->custom_mutators_count && afl->current_custom_fuzz) {
  ...
  if (fd >= 0) {
    ...
    ck_write(fd, mem, len, fn);  //IMP: this saves to filesystem!!!
    close(fd);
